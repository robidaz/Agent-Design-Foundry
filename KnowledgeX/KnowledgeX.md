...
- **Technology:** Python backend; Anthropic Claude API for reasoning; pgvector (local, via Docker Compose); Jinja2 for output rendering; embeddings run locally via `sentence-transformers` with `BAAI/bge-large-en-v1.5` (1024-dim, CPU). No third-party embedding API in the PoC.
...
>
> **Note:** To save cost for PoC I downloaded a 1.4GB local install sentence transformer. A production deployment may wish to leverage Voyage AI with an API Key

| Operation                                  | Model / Service                          | Est. Latency | Est. Cost / Run | Notes                               |
| ------------------------------------------ | ---------------------------------------- | ------------ | --------------- | ----------------------------------- |
| KB ingestion (one-time)                    | bge-large-en-v1.5 (local CPU) + pgvector | ~15–30s      | $0              | ~8 docs × avg 10 chunks each; local |
| Hybrid retrieval per question              | bge-large-en-v1.5 (local) + pgvector     | ~0.2s        | $0              | ~150ms query embed + ~50ms pgvector |
...
| **Total per engagement**                   |                                          | **~70–130s** | **~$0.15–0.30** | 30-question RFP; scales linearly    |
...
- Retrieval uses a local sentence-transformer for the query embedding plus pgvector — no third-party API call; Claude is called only at the coverage-judgment, agreement-judgment, and draft steps.
