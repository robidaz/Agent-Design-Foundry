...
**Desired outcome:** A prospective mid-market client self-initiates an assessment from Dynamix's website. The agent administers the discovery interview directly — a structured intake across six dimensions, with adaptive follow-up where evidence is thin — and produces a scored, evidence-backed draft readiness report with a 12/24/36-month roadmap. A Dynamix consultant enters only at the draft-review stage to calibrate, edit, and sign off before delivery. Consultant time shifts away from interview facilitation and concentrates on judgment.
> In my current role as Design Lead, when I'm securing funding for a project, the second most powerful tool I bring to a client pitch is a well-curated deck or document that cleanly lays out the proposed solution. The most powerful is a high-level working demo of an issue I have self-identified — something the client did not call out as a requirement, but that I discovered and solved on their behalf. Given that Dynamix has limited consultant availability and wants this assessment to scale, my recommendation is to bake a handful of deterministic questions into the intake so the agent can recommend a working demo of a problem Dynamix AI services can solve. Behind the scenes, Dynamix would maintain ~5 pre-prepared, templatable agents or lightweight AI-enabled tools. GreenLight would surface the recommendation, await consultant input, then call a sub-agent to craft and deploy the demo — which the consultant verifies before presenting it alongside the assessment report.
> > I will admit this recommendation loses value if Dynamix's clients have little in common with one another. If offered a position at Dynamix, I would re-evaluate once I had a better understanding of the existing client base.
>
> > ***Note:** Working-demo generation by sub-agents is out of scope for this document.*

**Dynamix Group Success signal:** Success should be defined beyond metrics alone in any client-facing application. Dynamix Group needs this assessment to demonstrate that the company can:
 - Execute a capable assessment of a company’s core business processes and identify operational strengths and weaknesses
 - Deliver a highly polished, client-facing roadmap that frames strengths as competitive differentiators and identified weaknesses as strategic opportunities for improvement — opportunities that Dynamix Group is prepared to support through professional services engagements
 - Create a customer confidence in Dynamix Group's ability to integrate AI into their business such that they become referenceable
   > One of the most important factors Manhattan considers is the referenceability of top-tier clients. Manhattan is willing to sacrifice hundreds of billable consulting hours if it means the client will be satisfied with the implementation and ultimately become a strong reference account

**Agent Success Signal:**
1. Supported score rate ≥ 85%: the percentage of dimension scores the consultant can trace to specific interview evidence without re-scoring from scratch.
2. Escalation rate ≤ 20%: the share of dimensions flagged as low-confidence or UNKNOWN per engagement. Both are proposed baselines pending Dynamix confirmation; both apply to agent-administered interviews, not pre-completed forms.
...
- Agent emits **structured markdown** as report content; a Python rendering function applies a Jinja template to produce the polished PDF roadmap document (with executive report graphics: score visualizations, roadmap timeline, tier callouts)
...
- Custom Relationship Management (CRM) integration
...
1. The agent administers the full discovery interview to the client via a dedicated Angular + Syncfusion intake UI hosted on Dynamix's website; there is no pre-completed form. The engagement is self-initiated by the client (likely per Dynamix recommendation).
2. The framework definition (dimensions, sub-criteria, rubric anchors, composite weights) is maintained by Dynamix in a versioned config file; the agent loads the active version at engagement start and has no write access.
...
5. The agent's report output is **structured markdown** — not the delivery artifact itself. The markdown is consumed as content input by a Python rendering function that applies a Jinja template and produces the **polished, client-facing roadmap document (PDF) with executive report graphics** (score visualizations, roadmap timeline, composite tier callouts). The consultant reviews and edits the markdown directly in this iteration; the Python renderer then re-generates the final PDF. There is no in-app consultant review workflow.
6. During testing synthetic discovery responses are used to simulate what a real client would enter through the production UI and exercise the full scoring range (1–5 across all dimensions).
...
8. Supported score rate (85%) and escalation rate (20%) are proposed — **require Dynamix confirmation** before the eval harness is built.
9. No data residency or air-gap requirements identified. Managed Cloud (Anthropic API) assumed. Because real client intake data flows through the system from day one a data classification and privacy review is a **production prerequisite**, not deferred to post-launch.
10. Client identity and engagement metadata (company name, employee count, revenue band, industry, respondent role) are captured in the first section of the intake itself. The agent uses this metadata to size and tailor the interview but does not authenticate the client.

### Constraints

- **Technology:** Python backend (orchestrator + scoring + report generation); Anthropic Claude API; Python rendering function using Jinja2 templates to transform the agent's structured markdown into a polished PDF roadmap document with executive report graphics; framework config in versioned JSON/YAML; Angular + Syncfusion (AI components) for the client-facing intake UI.
...
- **Autonomy:** The agent does not deliver reports to clients. Every report is reviewed, calibrated, and approved by a Dynamix Group consultant before client delivery. No auto-send.
...
flowchart TD
    ...
    L -- "structured markdown" --> M["Python Renderer<br/>(Jinja template + exec graphics)<br/>→ polished PDF"]
...
| Report renderer                    | Python function + Jinja2 template    | Consumes the agent's structured markdown as content input; applies Jinja template to produce the polished PDF roadmap document with executive report graphics (score visualizations, roadmap timeline, tier callouts). Markdown remains the editable intermediate; PDF is regenerated after consultant edits. |
...
7. **Render & Stage:** The agent emits the report as structured markdown. A Python rendering function consumes that markdown as content input, applies the Jinja template, and produces the polished PDF roadmap document with executive report graphics. The editable markdown and full follow-up transcript are routed to the consultant review queue; the PDF is regenerated from the markdown after consultant edits.
...
| Report render (Python + Jinja template)      | Jinja2 + Python   | ~1–2s        | Negligible      | Polished PDF (from agent's markdown content) + executive graphics |
