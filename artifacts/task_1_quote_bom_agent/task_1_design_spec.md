# Quote / BOM Generation Agent — Design Document
...
**Success signal:** (1) Reduction in quote turnaround time relative to the manual baseline. (2) Supported answer rate ≥ 90%: the percentage of BOM line items that a sales engineer can trace directly to a catalog entry and a ruleset clause without needing to validate from scratch. ***Note***: *answer rate will not be calculated in PoC*
